...
jhk.calendar = (function () {
  ...
  onChangeTeacher = function () {
    ...
    if (jqueryMap.$selectTeacher.val() == '-') {
      ...
      let motoData = jhk.model.getHenkou();
      stateMap.henkous = motoData.filter(jhkTeacherFilterF(jqueryMap.$selectTeacher.val()));
    }
  ...
  onChangeKyouka = function () {
    /* console.log("onChangeKyouka called"); */
    // configMap.teacherを消すタイミングがなかったせいで
    // 思わぬ操作(例えば教科を-にしたとき)で
    // 入力対象の教員が選択されるような挙動を見せていた。
    // 教科を変えたときに消しておく
    configMap.teacher = "";
